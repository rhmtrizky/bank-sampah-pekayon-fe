...
export default function CreatePengepulModal({ isOpen, onClose, onSuccess }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await pengepulService.create(payload);
      onClose();
      onSuccess?.();
      setName(""); setPhone(""); setAddress("");
    } catch (e: any) {
      setError(e?.response?.data?.message || e?.message || "Gagal menambah pengepul");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} className="max-w-[600px] p-5 lg:p-10">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="text-lg font-semibold">Tambah Pengepul</div>
