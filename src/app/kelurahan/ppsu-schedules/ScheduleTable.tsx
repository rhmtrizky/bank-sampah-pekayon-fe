...
export default function ScheduleTable({ kelurahanId }: { kelurahanId: number }) {
    ...
    const load = async () => {
      ...
      try {
        const [collections, withdraws] = await Promise.all<[
          ...
          kelurahanService.getCollectionSchedules(),
          kelurahanService.getWithdrawSchedules(),
        ]);

        const collectionRows: UIScheduleRow[] = collections
          .filter((c) => (c.kelurahan_id ?? kelurahanId) === kelurahanId)
          .map((c) => {
        ...
        const withdrawRows: UIScheduleRow[] = withdraws
          .filter((w) => (w.kelurahan_id ?? kelurahanId) === kelurahanId)
          .map((w) => {
  ...
  if (!rows.length)
    return (
      <EmptyState
        title="No schedules"
        description="Belum ada jadwal PPSU untuk kelurahan ini."
